...
FILESEXTRAPATHS:append = "${THISDIR}/linux-calixto-nxp_dts"

do_configure:append() {
    if [ "${MACHINE}" == "imx6ull-versa256" ]; then
        cp "${WORKDIR}"/imx6ull-calixto-versa256.dts "${S}arch/arm/boot/dts/imx6ull-calixto-versa.dts"
        dts_base="imx6ull-calixto-versa"        
    elif [ "${MACHINE}" == "imx6ull-versa512" ]; then
        cp "${WORKDIR}"/imx6ull-calixto-versa512.dts "${S}arch/arm/boot/dts/imx6ull-calixto-versa.dts"
        dts_base="imx6ull-calixto-versa"        
    elif [ "${MACHINE}" == "imx6ull-versa1024" ]; then
        cp "${WORKDIR}"/imx6ull-calixto-versa1024.dts "${S}arch/arm/boot/dts/imx6ull-calixto-versa.dts"
        dts_base="imx6ull-calixto-versa"
    fi
